 ...
 */
const AppBridgeProvider: React.FC<Props> = ({ children }: Props) => {
	...
	const [appBridgeConfig] = useState(() => {
		...
		return {
			...
			// more detail: https://community.shopify.com/c/shopify-cli-and-tools/shopify-app-bridge-react-provider-skip-app-installation/m-p/2200528/highlight/true#M3905
			forceRedirect: true,
		};
	});
	// console.log(appBridgeConfig);
	if (!import.meta.env.VITE_SHOPIFY_CLIENT_ID || !appBridgeConfig.host) {
		const bannerProps = !import.meta.env.VITE_SHOPIFY_CLIENT_ID
			? {
