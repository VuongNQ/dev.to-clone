...
const AutoAltImagesSEO = () => {
  const { t } = useTranslation();

  const refFeaturesUseAccordingPlan =
    useRef<IRefFeaturesUseAccordingPlan>(null);

  const { dataAltImgMetaTagState, handleSetDataAltImgMetaTagState ,isLoadFetchData} =
    useContext(SettingAltImgMetaTagContext);

  const eleDomEleHeader = useMemo(() => {
    const isActive = dataAltImgMetaTagState.auto_add_alt_image;

    return (
      <div className="flex gap-5 items-center">
        <div className="flex-1">
          <Text as="h4" variant="bodyMd" color="subdued">
          {t("smartSEO.alt_images.section_auto_setting.des")}
          </Text>
        </div>
  ...
  }, [dataAltImgMetaTagState.auto_add_alt_image, isLoadFetchData, t]);
    ...
    <FeaturesUseAccordingPlan
      ...
      onActionPrimary={() => {
        handleSetDataAltImgMetaTagState({
          auto_add_alt_image: !dataAltImgMetaTagState.auto_add_alt_image,
        });
      }}
...
export default AutoAltImagesSEO;
